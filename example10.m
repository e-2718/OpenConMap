...
%%
F={@(x) 0+0.*x,@(x) 0+0.*x,@(x) -1+(1-(x-1).^2).^0.5,@(y) 0+0.*y,@(y) 0+0.*y,@(x) -1+(1-(x+1).^2).^0.5,@(x) 0+0.*x,@(x) 0+0.*x,@(x) 1-(1-(x+1).^2).^0.5,@(y) 0+0.*y,@(y) 0+0.*y,@(x) 1-(1-(x-1).^2).^0.5};
X=[1,1.5,1, -i, -2i,-i,-1,-3,-1,i,2.5i,i];
...
h.FontSize=15;
%%
[C,err]=ConformalSolve(z_V,100,50,200,0.3,1e-4);
