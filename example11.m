...
%%
F={@(x) 0+0.*x,@(x) 0+0.*x,@(x) tan(pi*4/3).*x,@(x) tan(pi*4/3).*x,@(x) tan(pi*2/3).*x,@(x) tan(pi*2/3).*x};
X=[0,1,0,2*exp(i*pi*4/3),0,3*exp(i*pi*2/3)];
z_V=Initialize(F,X);
...
%%
[C,err]=ConformalSolve(z_V,500,500,2000,0.3,1e-4);
