...
%%
F={@(y) 2+0.*y,@(x) -1+0.*x,@(x) -1+(0.25-(x-1.0).^2).^0.5,@(x) -1+0.*x,@(x) -1-(0.25-(x+0.5).^2).^0.5,@(x) -1+0.*x,@(x) -0.5-(0.25-(x+1.5).^2).^0.5,@(y) -2+0.*y,@(x) 1-(1-(x+2).^2).^0.5,@(x) 1+0.*x,@(x) 1-x/2,@(y) 1-(0.25-(y-0.5).^2).^0.5,@(x) 0.5-(0.25-(x-1).^2).^0.5,@(x) 1-x/2};

X=[2,2-i,1.5-i,0.5-i,-i,-1-i,-1.5-i,-2-0.5i,-2,-1+i,i,(5 - sqrt(5))/5+(5 + sqrt(5))/10*i,1,(5 + sqrt(5))/5+(5 - sqrt(5))/10*i];
z_V=Initialize(F,X,30);
...
%%
[C,err]=ConformalSolve(z_V,100,500,2000,0.3,1e-4);
